    ...
    public function notificationShopify(Request $request){
        ...
        if(isset($request->id) && isset($request->fulfillments)){
            ...
            if(isset($request->note_attributes) && count($request->note_attributes) > 0){
                ...
                if($nota != "" && $shopifyData->rastreio != NULL){
                    $noteSend = new ImplementSendNoteOrderClient($shopifyData->id_mercadoLivre, "Pedido > ".  $nota . " - " . $shopifyData->rastreio, $shopifyData->id_vendedor);
                    $noteSend->send();
                }
            }
        }
