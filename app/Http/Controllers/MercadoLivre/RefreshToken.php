 ...
 **/
 ...
 class RefreshTokenController implements refreshToken
 {
     ...
     public function get($resource)
     {
         ...
         $acesso = token::where('user_id',$this->getUserId())->first();
         ...
         if ($DataSistema > $acesso->DataModify) {
             ...
             try {
                 token::where('user_id',$this->$this->getUserId())->update(['access_token' => $dados->access_token,'DataModify' => $this->getDataAtual()->format('Y-m-d H:i:s')]);
             } catch (\Exception $e) {
                   echo $e->getMessage();
             }
