...
class InterfaceClienteTiktokController implements ClienteController
{
    ...
    {
        try {
            if (order_site::VerificarVenda($result['order']['order_id']) == false) {
                // Tenta encontrar ou criar o pedido com base no campo numeropedido
                $pedidos = order_site::firstOrCreate(
                    ['numeropedido' => $result['order']['order_id']],
                    [
                        'local'               => 'TikTok Shop',
                        'valorVenda'          => $result['pagamento']['total'],
                        'valorProdutos'       => $result['pagamento']['total'],
           ...
           // Se o pedido foi recém-criado, insere os produtos e relaciona via pivot_site
            if ($pedidos->wasRecentlyCreated) {
                    if (product_site::getVerifyProduct($result['produto']['sku']) == true) {
                        // Cria o produto
                        $produto = new product_site();
                        $produto->nome       = $result['produto']['nome'];
                        $produto->codigo     = $result['produto']['sku'];
                        $produto->valor      = $result['produto']['preco_unitario'];
                        $produto->quantidade = $result['produto']['quantidade'];
                        $produto->seller_sku = $result['produto']['sku'];
                        $produto->image      = "-";
                        $produto->save();
